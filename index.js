...
console.log("GÖREV 1B: ", MenuElemaniOlustur("Kahve", 15, "Icecekler"));
console.log("GÖREV 1B: ", MenuElemaniOlustur("Soda", 15, "Icecekler"));
console.log("GÖREV 1B: ", MenuElemaniOlustur("Pizza", 40, "Pizzalar"));
...
console.log("GÖREV 2: ÖĞRENCİ: ", burger.indirim("öğrenci"));
console.log("GÖREV 2: DİĞER: ", burger.indirim("diğer"));
...
/* function degerlendirmeyazdir(oylesinebiseyyazabiliriz) {
  const x = [];
  for (let i = 0; i < oylesinebiseyyazabiliriz.length; i++) {
    if (oylesinebiseyyazabiliriz[i].isim === "Ahmet") {
      x.push(oylesinebiseyyazabiliriz[i].geribildirim);
    }
  }
  return x;
}
console.log("Ahmetin degerlendirmesi:", degerlendirmeyazdir(degerlendirmeler)); */

console.log(
  "GÖREV 3 -> Ahmetin degerlendirmesi: ",
  degerlendirmeler[5].geribildirim
);
...
console.log("GÖREV 4 -> REYNA'NIN DEGERLENDIRMESİ EKLENDI MI?: ", degerlendirmeler); */

console.log(
  "GÖREV 4 -> (önce) REYNA'NIN DEGERLENDIRMESİ: ",
...
console.log(
  "GÖREV 4 -> (sonra) REYNA'NIN DEGERLENDIRMESİ: ",
  degerlendirmeler[7].geribildirim
);
...
console.log(
  "GÖREV 5: EKLENEN YENİ DEĞERLENDİRME: ",
...
function AnahtardanDegerlendirmeAl(degerlendirmeArray, index) {
  return (
    degerlendirmeArray[index].isim +
    " isimli kişi " +
    degerlendirmeArray[index].puan +
    " puan verdi ve şunları yazdı: " +
    degerlendirmeArray[index].geribildirim
  );
}

console.log(
  "GÖREV 6: DEGERLENDİRME ALMA: ",
...
function SonDegerlendirmeyiAl(degerlendirmeArray) {
  const sonIndex = degerlendirmeArray.length - 1;
  const sonDegerlendirme = degerlendirmeArray[sonIndex];
...
console.log(
  "BONUS 1: PUANA GORE DEGERLENDİRMELER: ",
...
function UzunDegerlendirmeleriAl(degerlendirmeler) {
  //length, for loop, yeni bir değişkene uzunluk not alacağım
  ...
  for (let i = 0; i < degerlendirmeler.length; i++) {
    ...
    const uzunluk = degerlendirme.geribildirim.split(" ").length;
...
function arabaYapici(ilkKm) {
  return {
    kilometreSayaci: ilkKm,
    marka: "WIT",
    model: "2023",
    surus: function (gidilenKm) {
      this.kilometreSayaci += gidilenKm;
...
console.log("BONUS 3: ARABA YAPICI: ", araba1.surus(100));
